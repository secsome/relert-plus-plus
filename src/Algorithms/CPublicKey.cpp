...
const char* CPublicKey::pubkey_str = "AihRvNoIbTn85FZRYNZRcT+i6KpU+maCsEqr3Q5q+LDB5tH7Tz2qQ38V";

const char CPublicKey::char2num[0x100] =
{
...
CPublicKey::t_pubkey CPublicKey::pubkey;

CPublicKey::bignum CPublicKey::glob1;
uint32_t CPublicKey::glob1_bitlen;
uint32_t CPublicKey::glob1_len_x2;
uint32_t CPublicKey::glob2[130];
uint32_t CPublicKey::glob1_hi[4];
uint32_t CPublicKey::glob1_hi_inv[4];
uint32_t CPublicKey::glob1_hi_bitlen;
uint32_t CPublicKey::glob1_hi_inv_lo;
uint32_t CPublicKey::glob1_hi_inv_hi;

void CPublicKey::init_bignum(bignum n, uint32_t val, uint32_t len)
{
	memset(n, 0, len * 4);
	n[0] = val;
}

void CPublicKey::move_key_to_big(bignum n, const char* key, uint32_t klen, uint32_t blen)
{
...
void CPublicKey::key_to_bignum(bignum n, char* key, uint32_t len)
{
	uint32_t keylen;
	int i;
...
uint32_t CPublicKey::len_bignum(bignum n, uint32_t len)
{
	int i = len - 1;
	while (i >= 0 && n[i] == 0)
		i--;
...
uint32_t CPublicKey::bitlen_bignum(bignum n, uint32_t len)
{
...
void CPublicKey::init_pubkey()
{
	init_bignum(CPublicKey::pubkey.key2, 0x10001, 64);
	char keytmp[256];
	uint32_t i2 = 0;
...
uint32_t CPublicKey::len_predata()
{
	uint32_t a = (pubkey.len - 1) / 8;
	return (55 / a + 1) * (a + 1);
}

int CPublicKey::cmp_bignum(bignum n1, bignum n2, uint32_t len)
{
	n1 += len - 1;
	n2 += len - 1;
	while (len > 0)
...
void CPublicKey::mov_bignum(bignum dest, bignum src, uint32_t len)
{
	memmove(dest, src, len * 4);
}

void CPublicKey::shr_bignum(bignum n, uint32_t bits, uint32_t len)
{
	uint32_t i;
...
void CPublicKey::shl_bignum(bignum n, uint32_t bits, uint32_t len)
{
	uint32_t i;
	if (uint32_t i2 = bits / 32)
	{
...
uint32_t CPublicKey::sub_bignum(bignum dest, bignum src1, bignum src2, uint32_t carry, uint32_t len)
{
	len *= 2;
	while (--len != -1)
	{
		uint32_t i1 = *(unsigned short*)src1;
		uint32_t i2 = *(unsigned short*)src2;
		*(unsigned short*)dest = i1 - i2 - carry;
		src1 = (uint32_t*)(((unsigned short*)src1) + 1);
		src2 = (uint32_t*)(((unsigned short*)src2) + 1);
		dest = (uint32_t*)(((unsigned short*)dest) + 1);
		carry = ((i1 - i2 - carry) & 0x10000) ? 1 : 0;
	}
	return carry;
}

void CPublicKey::inv_bignum(bignum n1, bignum n2, uint32_t len)
{
	bignum n_tmp;
...
void CPublicKey::inc_bignum(bignum n, uint32_t len)
{
	while ((++ * n == 0) && (--len > 0))
		n++;
}

void CPublicKey::init_two_dw(bignum n, uint32_t len)
{
...
void CPublicKey::mul_bignum_word(bignum n1, bignum n2, uint32_t mul, uint32_t len)
{
	uint32_t tmp = 0;
	for (uint32_t i = 0; i < len; i++)
	{
		tmp = mul * (*(unsigned short*)n2) + *(unsigned short*)n1 + tmp;
		*(unsigned short*)n1 = tmp;
		n1 = (uint32_t*)(((unsigned short*)n1) + 1);
		n2 = (uint32_t*)(((unsigned short*)n2) + 1);
		tmp >>= 16;
	}
	*(unsigned short*)n1 += tmp;
}

void CPublicKey::mul_bignum(bignum dest, bignum src1, bignum src2, uint32_t len)
{
...
void CPublicKey::not_bignum(bignum n, uint32_t len)
{
	for (uint32_t i = 0; i < len; i++)
		*(n++) = ~*n;
}

void CPublicKey::neg_bignum(bignum n, uint32_t len)
{
	not_bignum(n, len);
	inc_bignum(n, len);
}

uint32_t CPublicKey::get_mulword(bignum n)
{
	unsigned short* wn = (unsigned short*)n;
...
void CPublicKey::dec_bignum(bignum n, uint32_t len)
{
	while ((-- * n == 0xffffffff) && (--len > 0))
		n++;
}

void CPublicKey::calc_a_bignum(bignum n1, bignum n2, bignum n3, uint32_t len)
{
	...
	if (g2_len_x2 >= glob1_len_x2)
	{
		...
		unsigned short* esi = ((unsigned short*)glob2) + (1 + g2_len_x2 - glob1_len_x2);
		unsigned short* edi = ((unsigned short*)glob2) + (g2_len_x2 + 1);
		for (; len_diff != 0; len_diff--)
		{
			edi--;
			unsigned short tmp = get_mulword((uint32_t*)edi);
			esi--;
			if (tmp > 0)
			{
...
void CPublicKey::calc_a_key(bignum n1, bignum n2, bignum n3, bignum n4, uint32_t len)
{
	bignum n_tmp;
...
void CPublicKey::process_predata(const byte* pre, uint32_t pre_len, byte* buf)
{
	bignum n2, n3;
...
void CPublicKey::get_blowfish_key(const byte* s, byte* d)
{
